lwt::ui::help_main() {
  ...
  echo "  lwt a my-feature --agents codex,claude \"fix...\" ${_lwt_dim}Launch multiple agents with one prompt${_lwt_reset}"
...
lwt::ui::help_add() {
  ...
  echo "  ${_lwt_bold}--agents list [prompt]${_lwt_reset}  ${_lwt_dim}Launch one or more agents from a comma-separated list${_lwt_reset}"
  ...
  echo "  ${_lwt_dim}Single-agent flags are shorthand for --agents with one item.${_lwt_reset}"
  echo "  ${_lwt_dim}Hyphen aliases like --claude-codex and --codex-gemini are supported.${_lwt_reset}"
  ...
  echo "  ${_lwt_dim}Multi-agent launches reuse the current shell for the first agent and open splits for the rest.${_lwt_reset}"
